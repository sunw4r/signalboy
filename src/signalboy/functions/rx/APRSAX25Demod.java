...
public class APRSAX25Demod extends RX {

    static int STATE_PACKET_SEARCH = 0;
    static int STATE_PACKET_ADDRESS = 1;
    ...
    private final String preamble = "01111110";
    ...
    public void processBuffer(short[] data) {
            ...
            for (int i = 0; i < bitsBuffer.length(); i++) {
                if (i + preamble.length() < bitsBuffer.length()) {
                    ...
                    if (preambleDetected.equals(preamble)) {
                        ...
                        preambleindex = i;
                        currentState = STATE_PACKET_ADDRESS;
                        break;
                    }
                }
            }
        ...
        if (currentState == STATE_PACKET_ADDRESS) {
        ...
        if (currentState == STATE_PACKET_MSGDECODING) {
            ...
            if (msgDecodeIndex + msgRest < bitsBuffer.length()) {
                ...
                //Acabamos de decodificar, vamos printar a mensage.
                StringBuilder sb = new StringBuilder(); // Some place to store the chars
                Arrays.stream( // Create a Stream
                ...
                String output = sb.toString();
                textAreaMessage.appendText("msg:" + output + "\n");
